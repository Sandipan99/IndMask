# Adopted from https://github.com/gzerveas/mvts_transformer/blob/master/src/models/ts_transformer.py
...
class TSTransformerEncoder(nn.Module):
    ...
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, repr_dim, dropout=0.1,
        ...
        self.repr_layer = nn.Linear(self.max_len*self.feat_dim, self.repr_dim)
    ...
    def forward(self, X, padding_masks):
        ...
        return self.repr_layer(output)


class TSTransformerEncoderClf(nn.Module):

    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, repr_dim, num_classes=6, dropout=0.1,
                 pos_encoding='fixed', activation='gelu', norm='BatchNorm', freeze=False):
        super(TSTransformerEncoderClf, self).__init__()
        ...
        self.cls_layer = nn.Linear(self.repr_dim, num_classes)
    ...
    def forward(self, X, padding_masks):
        ...
        output = self.act(output)  # the output transformer encoder/decoder embeddings don't include non-linearity
        #print(output.shape)
        #print("Finished until this")
        output = output.permute(1, 0, 2)  # (batch_size, seq_length, d_model)
        output = self.dropout1(output)
        # Most probably defining a Linear(d_model,feat_dim) vectorizes the operation over (seq_length, batch_size).
        output = self.output_layer(output)  # (batch_size, seq_length, feat_dim)
        output = output.reshape(batch_size, -1)
        output = self.repr_layer(output)
...
if __name__ == '__main__':
    ts_model = TSTransformerEncoder(feat_dim=3, max_len=206, d_model=128, n_heads=4, num_layers=4, dim_feedforward=1024, repr_dim=512)
    inp = torch.randn((4, 206, 3))
    padding_mask = torch.ones(4, 206).to(torch.int64)
    out = ts_model(inp, padding_mask)
    print(out.shape)